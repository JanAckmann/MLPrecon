...
do i=0,31
  max_ind=0
  Layer1(i+1,177,1)=0.0d0
  do k=2,176,7
   max_ind=max_ind+1
   ! optimized version is based on only only 5 input values
   If (max_ind ==3 .or. max_ind ==11 .or. max_ind ==13 .or. max_ind ==15 .or. max_ind ==23) then
   !If (max_ind ==13 ) then
        Layer1(i+1,k,1)=Layer1(i+1,k,1)

   else
        Layer1(i+1,k,1)=0.0d0
   endif
...
! make sure all other weight fields are set to 0
do i=0,31
Layer1(i+1,1,1)=0.0d0
enddo
...
do ID_PREC=5,5,-5  !! preconditioner, later overwritten in linear solver: "gcr_pre"
  ...
  do DP_relax=0,0     !! no polar filtering
   ...
   EXP_NAME= 'data/data_ADI_NOTgcr_EXP3_Dp_1M10_MLPrecon_iterINF_REDU_res05'
