...
do ID_PREC=0,0,-5  !! preconditioner, later overwritten in linear solver: "gcr_pre"
...
if (iprint==1) then
    call write_residual(r_HP,eps*Exit_cond, 0, TIME, codesQ, codes, IRHW, DX_rpe, DY_rpe,&
...
! Apply Machine-learned preconditioner

!do J=0,n2-1
!   NN_r_HP(:,J+1)= (r_HP(2:n1-1,n2-J))
!enddo
...
! Apply implicit richardson preconditioner
...
do it=1,itr
 ...
 if (iprint==1) then
    call write_residual(r_HP,eps*Exit_cond, it, TIME, codesQ, codes, IRHW, DX_rpe, DY_rpe, n1, n2, 52, ID_prec ,EXP_NAME)
endif


!write(*,*) it, maxval(ABS(r_HP(:,:))), eps*err0
!read(*,*)
...
enddo
write(*,*) niter
 write(*,*) 'truth ACC',maxval(ABS(r_true(:,:)))/maxval(ABS(r0_true(:,:))),'max(rtrue)' ,&
...
! 2) prepare preconditioner coefficients
...
Do J=0,M-1
  ...
  do I=0,N-1
      ...
      lat_box=-2
           ...
           ML_input0(16)=  NNr_HP(pos_lon+1, pos_lat+1)
      ...
      lat_box=0
           ...
           ML_input0(72)=  NNr_HP(pos_lon+1, pos_lat+1)
      ...
      lat_box=0
           ...
           ML_input0(86)=  NNr_HP(pos_lon+1, pos_lat+1)
      ...
      lat_box=0
           ...
           ML_input0(100)=  NNr_HP(pos_lon+1, pos_lat+1)
      ...
      lat_box=0
           ...
           ML_input0(156)=  NNr_HP(pos_lon+1, pos_lat+1)
...
!  do I=1,In_dim-1
!    write(*,*) I, Input(I), Weights(I,1)
!  end do
!read(*,*)
do J=1,Out_dim

  summand=0.0d0
    summand=summand+Input(16)*Weights(16,J)
    summand=summand+Input(72)*Weights(72,J)
    summand=summand+Input(86)*Weights(86,J)
    summand=summand+Input(100)*Weights(100,J)
    summand=summand+Input(156)*Weights(156,J)
  !do I=1,In_dim-1
  !  summand=summand+Input(I)*Weights(I,J)
  !end do
  !summand=summand+Weights(In_dim,J)   !! add the bias
